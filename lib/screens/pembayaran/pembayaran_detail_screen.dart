...
class PembayaranDetailScreen extends StatefulWidget {
  final int id;
  const PembayaranDetailScreen({super.key, required this.id});
...
class _PembayaranDetailScreenState extends State<PembayaranDetailScreen> {
  ...
  Future<void> _loadDetail() async {
    ...
    try {
      _data = await Provider.of<PembayaranProvider>(context, listen: false)
          .getDetail(widget.id);
    } catch (e) {
      _error = e.toString();
    }
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
            ...
            child: Padding(
              ...
              child: Column(
                  ...
                  children: [
                    ...
                    _InfoRow('Metode', d.metodePembayaran ?? '-'),
                    _InfoRow('Tanggal', Formatters.date(d.tglPembayaran)),
                    if (d.penjualan != null)
                      _InfoRow('Invoice', d.penjualan!['nomor'] ?? '-'),
            ...
            child: Padding(
              ...
              child: Row(
                  ...
                  children: [
                    ...
                    const SizedBox(width: 8),
                    Flexible(
                      child: Text(Formatters.currency(d.jumlahBayar ?? 0),
                          style: const TextStyle(
                              fontSize: 18,
                              fontWeight: FontWeight.bold,
          ...
          else if (d.buktiBayar != null && d.buktiBayar!.isNotEmpty)
            LampiranSection(paths: [d.buktiBayar!]),
        ],
      ),
    );
  }
  ...
  Future<void> _handleAction(String action) async {
    ...
    try {
      if (action == 'approve') {
        await provider.approvePembayaran(widget.id);
        if (mounted) {
        ...
        await provider.cancelPembayaran(widget.id);
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
        ...
        await provider.uncancelPembayaran(widget.id);
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
              content: Text('Pembayaran berhasil di-uncancel.'),
              backgroundColor: Colors.green));
        }
      }
      await _loadDetail();
